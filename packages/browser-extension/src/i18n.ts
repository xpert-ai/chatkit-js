...
const MESSAGES = {
  en: {
    ...
    automationProvider: 'Automation target',
    currentTabProvider: 'Current Chrome tab',
    isolatedRunnerProvider: 'Isolated browser runner',
    isolatedBrowser: 'Isolated browser',
    isolatedBrowserHelp:
      'Starts a separate visible Chrome window with a temporary profile and download directory. Existing Chrome login state is not copied.',
    refreshRunnerStatus: 'Refresh status',
    runnerStatusChecking: 'Checking browser runner status…',
    runnerStatus: 'Browser runner',
    runnerUnavailable: 'Browser runner unavailable',
    width: 'Width',
    height: 'Height',
    position: 'Position',
  ...
  'zh-Hans': {
    ...
    automationProvider: '自动化目标',
    currentTabProvider: '当前 Chrome 标签页',
    isolatedRunnerProvider: '隔离浏览器 Runner',
    isolatedBrowser: '隔离浏览器',
    isolatedBrowserHelp:
      '启动使用临时 profile 和独立下载目录的可见 Chrome 窗口，不复制当前 Chrome 的登录状态。',
    refreshRunnerStatus: '刷新状态',
    runnerStatusChecking: '正在检查 Browser Runner 状态…',
    runnerStatus: 'Browser Runner',
    runnerUnavailable: 'Browser Runner 不可用',
    width: '宽度',
    height: '高度',
    position: '位置',
    bottomRight: '右下',
