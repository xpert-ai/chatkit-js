...
AI and human messages display their server `updatedAt` below the content, next to available actions. Timestamps appear when the message is hovered or keyboard focus is inside it; hovering or focusing the timestamp shows the full date, time, and timezone. The compact `HH:mm` uses the viewer's local timezone. The latest AI message's action buttons remain visible, while earlier actions appear on hover or focus (and stay accessible on touch layouts). Missing or invalid timestamps are omitted, and streaming AI messages wait until completion. Server acknowledgement and completion timestamps survive later conversation snapshots and history reloads. This footer is also available without `collapseProcess`.

Today's timestamps show only `HH:mm`. Earlier dates within the past week include a localized relative day, such as **昨天 16:13**, **前天 16:13**, or **3 days ago 16:13**. Older messages include month/day, and also the year when different from the current year. Day boundaries follow the viewer's local calendar, including across daylight saving changes.
